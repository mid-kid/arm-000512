...
s_thumb_set (equiv)
     int equiv;
{
  /* XXX the foloowing is a duplicate of the code for s_set() in read.c
     ...
     fixS * fixp;
{
  ...
  switch (fixp->fx_r_type)
    ...
    case BFD_RELOC_ARM_LITERAL:
    case BFD_RELOC_ARM_HWLITERAL:
      /* If this is called then a literal has been referenced across
  ...
  switch (c)
	...
	default:
	  ...
	  switch (*str)
	    ...
	    case '8':
	      if (! strcmp (str, "8") || ! strcmp (str, "810"))
		...
		cpu_variant = (cpu_variant & ~ARM_ANY) | ARM_9 | ARM_ARCHv4 | ARM_LONGMUL | ARM_THUMB;
	      else if (! strcmp (str, "920"))
		cpu_variant = (cpu_variant & ~ARM_ANY) | ARM_9 | ARM_ARCHv4 | ARM_LONGMUL;
	      else if (! strcmp (str, "920t"))
		cpu_variant = (cpu_variant & ~ARM_ANY) | ARM_9 | ARM_ARCHv4 | ARM_LONGMUL | ARM_THUMB;
	      else if (! strcmp (str, "9tdmi"))
		cpu_variant = (cpu_variant & ~ARM_ANY) | ARM_9 | ARM_ARCHv4 | ARM_LONGMUL | ARM_THUMB;
	      else
		goto bad;
	      break;
	    ...
	    case 's':
	      if (   ! strcmp (str, "strongarm")
		  || ! strcmp (str, "strongarm110")
		  || ! strcmp (str, "strongarm1100"))
		cpu_variant = (cpu_variant & ~ARM_ANY) | ARM_8 | ARM_ARCHv4 | ARM_LONGMUL;
	      else
		goto bad;
	      break;
...
arm_cleanup ()
{
  if (current_poolP != NULL)
    {
      /* If the pool has been assigned to a segment, switch to it.  */
      if (S_GET_SEGMENT (current_poolP) != NULL
	  && SEG_NORMAL (S_GET_SEGMENT (current_poolP)))
	subseg_set (S_GET_SEGMENT (current_poolP), 0);
