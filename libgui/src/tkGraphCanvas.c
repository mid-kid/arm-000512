...
static Layout_Graph *
GetGraphLayoutII(TkCanvas *canvasPtr);
static
int
    ...
    int i;
{
    ...
    char* argv[8];
    Layout_Graph *graph=GetGraphLayoutII(canvasPtr);
...
static Layout_Graph *
GetGraphLayout(canvCmd)
     Tcl_CmdInfo *canvCmd;
{
    Tcl_HashEntry *entry;
    entry = Tcl_FindHashEntry(&graphTable, (char *)canvCmd->clientData);
    if (entry)
...
static Layout_Graph *
GetGraphLayoutII(canvasPtr)
     TkCanvas *canvasPtr;
{
    Tcl_HashEntry *entry;
    entry = Tcl_FindHashEntry(&graphTable, (char *)canvasPtr);
    if (entry)
	return (Layout_Graph *)Tcl_GetHashValue(entry);
     ...
     Layout_Graph **graph;
{
    *graph = GetGraphLayout(canvCmd);
	...
	if (createcanvasgraph(interp, canvCmd, graph) != TCL_OK)
	    return TCL_ERROR;
	newitem = Tcl_CreateHashEntry(&graphTable, 
     ...
     char **argv;
{
	...
	for (i = 3; i < argc; i++) {
	    ...
	    Layout_Graph *graph = GetGraphLayout(&canvCmd);
	    if (graph)
	    ...
	    */
	    ...
	    if (graph) {
		Tcl_HashEntry *entry;
		entry = Tcl_FindHashEntry(&graphTable, 
...
static void 
delete_graph_command(ClientData clientData)
{
    Tcl_DeleteHashTable(&graphTable);
}
...
create_graph_command(Tcl_Interp *interp)
{
    ...
    if (Tcl_CreateCommand(interp, "graph", GraphCanvasCmd, 
			  NULL, delete_graph_command) == NULL)
	return TCL_ERROR;
