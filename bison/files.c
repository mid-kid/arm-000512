...
const char **fparser_internal = NULL;
extern const char *internal_bison_simple[];
extern const char *internal_bison_hairy[];
...
openfiles()
{
  ...
  if (! noparserflag) 
    {
      /* CYGNUS LOCAL: internalize bison.* files */
      int allow_internal_file = 0;
      if (simple_parser != NULL)
	filename = simple_parser;
      else if ((filename = getenv("BISON_SIMPLE")) == NULL)
	{
	  allow_internal_file = 1;
	  parsername = PFILE;
	  
	  if (bison_library != NULL)
	    libname = bison_library;
	  else if ((libname = getenv ("BISONLIB")) == NULL)
	    libname = PLIB;

	  filename = (char *) xmalloc (strlen (libname)
				       + strlen (parsername)
				       + 1);
	  strcpy (filename, libname);
	  strcat (filename, parsername);
	}
#ifdef MSDOS
      /* File doesn't exist in current directory; try in INIT directory.  */
      cp = getenv("INIT");
      if ((filename == 0 || stat (filename, &tmp_statbuf) < 0) && cp != NULL)
        {
	  allow_internal_file = 1;
	...
	fparser_internal = internal_bison_simple;
      else
  ...
  if (! noparserflag) 
    {
      /* CYGNUS LOCAL: internalize bison.* files */
      int allow_internal_file = 0;
      if (hairy_parser != NULL)
	filename = hairy_parser;
      else if ((parsername = getenv ("BISON_HAIRY")) == NULL)
	{
	  allow_internal_file = 1;
	  filename = PFILE1;
	...
	filename = PFILE1;
      if (stat (filename, &tmp_statbuf) < 0)
	fparser_internal = internal_bison_hairy;
      else
	fparser = tryopen(filename, "r");
    }
