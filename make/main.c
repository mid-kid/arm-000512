...
/* CYGNUS LOCAL: use path_separator_char variable instead of
...
#ifdef __CYGWIN32__
    { 4, flag, (char *) &unixy_shell, 1, 1, 0, 0, 0,
        "unix", 0,
      "Run in UNIX mode (use sh.exe subshell)" },
    { 5, flag_off, (char *) &unixy_shell, 1, 1, 0, 0, 0,
        "win32", 0,
...
/* CYGNUS LOCAL: if WINDOWS32 or __CYGWIN32__ */
...
find_and_set_default_shell(char *token)
{
  ...
  /* CYGNUS LOCAL: FIXME: is this change appropriate? */
#ifdef __CYGWIN32__
  if (0) {
#else
  if (!no_default_sh_exe &&
    ...
    if (v && v->value) {
      ...
      p  = v->value;
      /* CYGNUS LOCAL: path_separator_char, not PATH_SEPARATOR_CHAR */
...
#endif
{
...
#ifdef __CYGWIN32__
  {
    ...
    if (make_mode_env)
      if (strcaseequ (make_mode_env, "UNIX"))
	unixy_shell = 1;
  }
#endif /* __CYGWIN32__ */
...
/* CYGNUS LOCAL: or for Cygwin */
  ...
  if (unixy_shell)
    ...
    {
      path_separator_char = ';';
      /* set default shell to native command shell */
  ...
  /* CYGNUS LOCAL: properly quote the value of $(MAKE_COMMAND).  */
  {
    char *quoted, *p;
    
    quoted = xmalloc (2 * strlen (argv[0]));
    p = quote_as_word (quoted, argv[0], 0);
    *p = '\0';
    (void) define_variable ("MAKE_COMMAND", 12, quoted, o_default, 0);
    free (quoted);
  }
...
/* CYGNUS LOCAL: Cygwin */
#ifdef __CYGWIN32__
  /* FIXME: Isn't no_default_sh_exe always zero? */
  if (unixy_shell && no_default_sh_exe)
    no_default_sh_exe = !find_and_set_default_shell(NULL);
#endif
