...
Copyright (C) 1988,89,91,92,93,94,95,96,97,99 Free Software Foundation, Inc.
  ...
  if (*p != '\0')
    {
	 ...
	 will still be nil if P contains no existing directories.  */
      ...
      if (!cygwin32_posix_path_list_p (p))
        {
          posixp = (char *)
                   alloca (cygwin32_win32_to_posix_path_list_buf_size (p));
          cygwin32_win32_to_posix_path_list (p, posixp);
          p = posixp;
        }
#endif /* __CYGWIN32__ */
      /* END CYGNUS LOCAL */
