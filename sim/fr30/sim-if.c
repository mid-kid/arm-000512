     ...
     char **argv;
{
  ...
  STATE_OPCODE_TABLE (sd) = fr30_cgen_opcode_open (STATE_ARCHITECTURE (sd)->mach,
						   CGEN_ENDIAN_BIG);
  fr30_cgen_init_dis (STATE_OPCODE_TABLE (sd));
...
sim_close (sd, quitting)
     ...
     int quitting;
{
  fr30_cgen_opcode_close (STATE_OPCODE_TABLE (sd));
  sim_module_uninstall (sd);
}

SIM_RC
