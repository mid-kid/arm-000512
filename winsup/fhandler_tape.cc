...
{
  return ((win_error == ERROR_END_OF_MEDIA)
...
{
  return ((win_error == ERROR_FILEMARK_DETECTED)
...
int
fhandler_dev_tape::linearize (unsigned char *buf)
{
  int len = sizeof (norewind);

  if (! buf)
    {
      len += fhandler_dev_raw::linearize (buf);
      return len;
    }
  memcpy (buf, (char *) &norewind, sizeof (norewind));
  buf += sizeof (norewind);

  len += this->fhandler_dev_raw::linearize (buf);
  return len;
}

int
fhandler_dev_tape::de_linearize (const unsigned char *buf)
{
  const unsigned char *orig_buf = buf;

  memcpy ((char *) &norewind, buf, sizeof (norewind));
  buf += sizeof (norewind);

  int len = buf - orig_buf;
  return (len + this->fhandler_dev_raw::de_linearize (buf));
}
