...
LPCRITICAL_SECTION ResourceLocks::
Lock (int _resid)
{
  if (!inited)
    {
...
void MTinterface::
ReleaseItem (MTitem * _item)
{
  _item->used = false;
};

MTitem *MTinterface::
Find (void *_value, int (*comp) (void *, void *), register int &_index, MTList * _list)
{
...
int MTinterface::
Find (MTitem & _item, MTList * _list)
{
...
int MTinterface::
FindNextUnused (MTList * _list)
{
  register int i = 0;
  for (; i < _list->index && _list->items[i] != NULL && _list->items[i]->used; i++);
  return i;
};

MTitem *MTinterface::
GetItem (int _index, MTList * _list)
{
  return (_index < _list->index ? _list->items[_index] : NULL);
};

MTitem *MTinterface::
SetItem (int _index, MTitem * _item, MTList * _list)
{
...
ThreadItem *MTinterface::
CreateThread (pthread_t * t, TFD (func), void *arg, pthread_attr_t a)
{
...
SemaphoreItem *MTinterface::
GetSemaphore (sem_t * sp)
{
...
SemaphoreItem *MTinterface::
CreateSemaphore (sem_t * _s, int pshared, int _v)
{
