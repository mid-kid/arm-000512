...
int
fhandler_dev_raw::linearize (unsigned char *buf)
{
  int len = sizeof (devbufsiz) + sizeof (unit);

  if (!buf)
    {
    len += fhandler_base::linearize (buf);
      return len;
    }
  memcpy (buf, (char *) &devbufsiz, sizeof (devbufsiz));
  buf += sizeof (devbufsiz);
  memcpy (buf, (char *) &unit, sizeof (unit));
  buf += sizeof (unit);
...
int
fhandler_dev_raw::de_linearize (const unsigned char *buf)
{
  const unsigned char *orig_buf = buf;

  memcpy ((char *) &devbufsiz, buf, sizeof (devbufsiz));
  buf += sizeof (devbufsiz);
  memcpy ((char *) &unit, buf, sizeof (unit));
  buf += sizeof (unit);
