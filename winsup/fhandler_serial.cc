...
fhandler_serial::raw_read (void *ptr, size_t ulen)
{
  ...
  debug_printf ("ulen %d, vmin_ %d, vtime_ %d, hEvent %p", ulen, vmin_, vtime_, io_status.hEvent);
  if (!overlapped_armed)
...
int
fhandler_serial::de_linearize (const unsigned char *buf)
{
  int res = fhandler_base::de_linearize (buf);
  overlapped_setup ();
  debug_printf ("io_status.hEvent %p", io_status.hEvent);
  return res;
}
