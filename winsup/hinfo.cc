  ...
  if (!myself->parent_alive && NOTSTATE(myself, PID_CYGPARENT))
    {
      ...
      dtable.init_std_file_from_handle (0, GetStdHandle (STD_INPUT_HANDLE),
					GENERIC_READ, "{stdin}");
...
hinfo::build_fhandler (int fd, DWORD dev, const char *name, int unit)
{
  ...
  switch (dev & FH_DEVMASK)
    {
      case FH_TTYM:
	fh = new (buf) fhandler_tty_master (name);
	break;
      case FH_CONSOLE:
      case FH_CONIN:
      case FH_CONOUT:
...
{
  int res = -1;
  fhandler_base *newfh = NULL, *oldfh;	// = NULL to avoid an incorrect warning
  extern void set_std_handle (int);
  ...
  oldfh = fds[oldfd];
  newfh = build_fhandler (-1, oldfh->get_device (), NULL);
  *newfh = *oldfh;
  oldfh->dup (newfh);
      ...
      if (*buf == 0xff)
	{
	 ...
	 this to build a new fhandler type. */
      ...
      debug_printf ("fd %d, name %s, win32 name %s, status %p",
		    i, obuf, win32, status);
      build_fhandler (i, status, (const char *) NULL)->set_name ((char *)obuf, win32);
      len = fds[i]->de_linearize ((unsigned char *)buf);
...
void
hinfo::fixup_after_fork ()
{
  HANDLE parent;

  first_fd_for_open = 0;
  if (!(parent = OpenProcess (PROCESS_ALL_ACCESS, FALSE,
			     procinfo(myself->ppid)->dwProcessId)))
    {
      system_printf ("unable to contact parent process");
      return;
    }
  fhandler_base *fh;
