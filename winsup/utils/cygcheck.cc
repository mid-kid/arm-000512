...
int
mntsort (const void *va, const void *vb)
{
  struct mntent *a = (struct mntent *) va;
  struct mntent *b = (struct mntent *) vb;
...
dump_sysinfo ()
{
  ...
  for (i = 0; i < 26; i++)
	...
	case VER_PLATFORM_WIN32_NT:
	  ...
	  break;
	case VER_PLATFORM_WIN32_WINDOWS:
	  better_call = (osversion.dwBuildNumber > 1000);
	  break;
	}
      ...
      if (better_call)
	{
	  longlong free_me = 0, free_bytes = 0, total_bytes = 1;
	  HINSTANCE k32 = LoadLibrary ("kernel32.dll");
	  FARPROC gdfse = GetProcAddress (k32, "GetDiskFreeSpaceExA");
	  if (gdfse)
	    if (gdfse (drive, (PULARGE_INTEGER *) & free_me,
      ...
      else
	{
      ...
      printf ("%s %s %-6s ", drive, drive_type, fsname);
      if (capacity_mb >= 0)
  ...
  struct mntent *mnt;
  struct mntent *ments = (struct mntent *) malloc (10 * sizeof (struct mntent));
  int maxents = 10, numents = 0;
  setmntent (0, 0);
  ...
  while ((mnt = getmntent (0)))
	...
	{
	  maxents += 10;
	  ments = (struct mntent *) realloc (ments,
					     maxents * sizeof (struct mntent));
	}
      ments[numents] = *mnt;
      numents++;
#define M(f,m) if (m<strlen(mnt->f)) m=strlen(mnt->f)
      M (mnt_fsname, ml_fsname);
      M (mnt_dir, ml_dir);
      M (mnt_type, ml_type);
#undef M
    }

  qsort (ments, numents, sizeof (struct mntent), mntsort);
