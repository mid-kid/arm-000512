...
fillout_pinfo (DWORD pid)
{
  ...
  if ((nextpid = !!(pid & CW_NEXTPID)))
    pid ^= CW_NEXTPID;
  for (i = 0; i < cygwin_shared->p.size(); i++)
    if (!pid || (DWORD) cygwin_shared->p.vec[i].pid == pid)
      {
	if (nextpid && pid)
	  {
	    pid = 0;
	...
	else if (cygwin_shared->p.vec[i].pid)
	  {
...
//ep.dwSpawnedProcessId = p->dwSpawnedProcessId;
  ...
  ep.strace_mask = p->strace_mask;
  ep.strace_file = p->strace_file;
