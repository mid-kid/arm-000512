...
/* HACK ALERT! Removed the static - need to find a real abstraction for getting
   this info out of remote.c. */
int remote_get_threadinfo PARAMS ((threadref *threadid,
...
/* Should we try the 'z'/'Z' requests?  If this is set to one when the
   stub doesn't support 'z'/'Z', the only consequence is some
   unecessary traffic. */
static int stub_supports_Z = 1;
...
/* Removed the static.  This whole way of getting extra thread info
   is unclean, and a better way to do it must be found... */
...
serial device is attached to the remote system (e.g. /dev/ttya).");
  ...
  /* Start out by trying the 'Z' request for stub-managed breakpoints
  ...
  if (stub_supports_Z) 
    {
      ...
      *(p++) = ',';
      p += hexnumstr (p, (ULONGEST) addr);
#if defined GET_BREAKPOINT_SIZE
      bp_size = GET_BREAKPOINT_SIZE (addr);
      sprintf (p, ",%d", bp_size);
#else
      *p = '\0';
#endif
	...
	return (buf[0] == 'E');
      ...
      /* The stub does not support the 'Z' request. */
      stub_supports_Z = 0;
    }
