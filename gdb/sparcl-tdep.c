  ...
  if (serial_flag)
  ...
  else
    {
      ...
      /* Setup the socket.  Must be raw UDP.
         If the hostname is numeric, don't use gethostbyname();
	 the Cygwin implementation will try to use DNS, which could hang
	 GDB on private networks with no name servers. */

      if (p[0] >= '0' && p[0] <= '9')
	sockaddr.sin_addr.s_addr = inet_addr (p);
      else
	{
	  struct hostent *he = gethostbyname (p);
